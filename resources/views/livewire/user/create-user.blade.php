<div class="event-page member-page">
    ...
    <x-page-header
        title="Add a member"
        subtitle="Create a member profile, record contact details, and set account access."
        :backRoute="route('users.index')"
        backLabel="Members" />

    <form wire:submit="save" class="member-editor-layout">
        <aside class="event-editor-intro member-editor-intro" aria-label="Member setup guide">
            <span class="event-eyebrow">Member setup</span>
            <h2>Welcome someone into the community.</h2>
            <p>Build a complete profile so ministry teams can communicate, provide care, and organize participation.</p>
            <div class="event-editor-note">
                <x-heroicon-o-user-plus aria-hidden="true" />
                <p>A unique QR identity is generated automatically after saving.</p>
            </div>
        </aside>
        ...
        <div class="member-form-stack">
            <section class="event-form-panel" aria-labelledby="member-identity-title">
                ...
                <div class="event-form member-fields">
                    <div class="event-field event-field-wide">
                        <label for="name">Full name <span aria-hidden="true">*</span></label><p class="event-field-hint">Enter the member’s complete name.</p>
                        <input type="text" id="name" wire:model="name" placeholder="e.g. Maria Santos" autocomplete="name" class="@error('name') is-invalid @enderror">
                    ...
                    <div class="event-field">
                        ...
                        <input type="email" id="email" wire:model="email" placeholder="name@example.com" autocomplete="email" class="@error('email') is-invalid @enderror">
                        @error('email')<p class="event-field-error" role="alert">{{ $message }}</p>@enderror
                    </div>
                    <div class="event-field">
                        <label for="phone">Phone number</label><p class="event-field-hint">Include the country code when possible.</p>
                    ...
                    <div class="event-field">
                        ...
                        @error('gender')<p class="event-field-error" role="alert">{{ $message }}</p>@enderror
                    </div>
                    <div class="event-field">
                        <label for="birthdate">Birthdate</label><p class="event-field-hint">Used for member records and sign-in.</p>
                        <input type="date" id="birthdate" wire:model="birthdate" autocomplete="bday" class="@error('birthdate') is-invalid @enderror">
                        @error('birthdate')<p class="event-field-error" role="alert">{{ $message }}</p>@enderror
                    </div>
                </div>
            </section>
            ...
            <section class="event-form-panel member-location-panel" aria-labelledby="member-location-title">
                ...
                <div class="member-location-body">
                    <p class="member-section-intro">Choose an address and confirm the map position. This helps pastoral teams understand where members are located.</p>
                    <livewire:components.address-map-picker
                        :region-code="$regionCode" :province-code="$provinceCode" :city-code="$cityCode"
                        :barangay-code="$barangayCode" :street-address="$streetAddress"
            ...
            <section class="event-form-panel" aria-labelledby="member-account-title">
                ...
                <div class="event-form member-fields">
                    <div class="event-field">
                        <label for="password">Password <span aria-hidden="true">*</span></label><p class="event-field-hint">Create the member’s initial password.</p>
                        <input type="password" id="password" wire:model="password" placeholder="Enter password" autocomplete="new-password" class="@error('password') is-invalid @enderror">
                        @error('password')<p class="event-field-error" role="alert">{{ $message }}</p>@enderror
                    </div>
                    <div class="event-field">
                ...
                <div class="event-form-actions member-form-actions">
                    <a href="{{ route('users.index') }}" class="event-button-secondary" wire:navigate>Cancel</a>
                    <button type="submit" class="event-button-primary" wire:loading.attr="disabled" wire:target="save"><span wire:loading.remove wire:target="save">Create member</span><span wire:loading wire:target="save">Creating…</span><x-heroicon-o-chevron-right wire:loading.remove wire:target="save" /></button>
                </div>
            </section>
        </div>
    </form>
