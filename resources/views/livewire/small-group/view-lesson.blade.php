<div class="event-page group-page lesson-detail-page">
    ...
    <x-page-header
        ...
        backLabel="Lessons">
        <x-slot:actions>
            @can('lessons.update')<a href="{{ route('small-groups.lessons', $lesson->smallGroup) }}?edit={{ $lesson->id }}" class="event-button-secondary" wire:navigate>
    ...
    <section class="event-detail-hero lesson-hero" aria-label="Lesson overview">
        ...
        @can('lesson_progress.view')<div class="lesson-completion-ring" style="--completion: {{ $totalMembers > 0 ? round(($completedCount / $totalMembers) * 100) : 0 }}%">
            <div><strong>{{ $totalMembers > 0 ? round(($completedCount / $totalMembers) * 100) : 0 }}%</strong><small>complete</small></div>
        </div>@endcan
    </section>

    <div @class(['lesson-layout', 'lesson-layout-content-only' => Gate::denies('lesson_progress.view')])>
        ...
        @can('lesson_progress.view')<aside class="lesson-progress-summary" aria-labelledby="lesson-summary-title">
    ...
    @can('lesson_progress.view')<section class="event-attendance-panel lesson-member-progress" aria-labelledby="member-progress-title">
            ...
            <ul class="lesson-progress-list">
                @foreach($lesson->smallGroup->members as $member)
                    ...
                    <li>
                        ...
                        @can('lesson_progress.update')<div class="lesson-progress-controls" aria-label="Progress for {{ $member->user->name }}">
                        ...
                        </div>@else<span class="group-status">{{ ucwords(str_replace('_', ' ', $currentStatus)) }}</span>@endcan
                    </li>
                @endforeach
            </ul>
        @else
